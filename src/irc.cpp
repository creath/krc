...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #KRCoinTEST2\r");
            Send(hSocket, "WHO #KRCoinTEST2\r");
        } else {
            // randomly join #KRCoin00-#KRCoin05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #KRCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #KRCoin%02d\r", channel_number).c_str());
        }
