...
const string strMessageMagic = "KRCoin Signed Message:\n";
...
int64_t GetProofOfWorkReward(unsigned int nBits, int64_t nFees, int nHeight)
{
	...
	if (nHeight == 1)
		return 2900000000 * COIN;
    
	// Force block reward to zero when right shift is undefined.
    int halvings = nHeight / 8000000;
    if (halvings >= 64)
        return nFees;
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in KRCoin
...
bool CBlock::CheckBlock(bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig) const
{
    ...
    if (fProofOfStake)
    {
        ...
        // KRCoin: check proof-of-stake block signature
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "KRCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "FTC Genesis";
        CTransaction txNew;
        txNew.nTime = 1467305460;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1467305462;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 159898; // !fTestNet ? 1575379 : 46534;
		

        block.print();

        //// debug print
        assert(block.hashMerkleRoot == uint256("0x27ea1d7f591f916ffd1406278c2bd4a3fba0cd9a4c4bb6f141e252e5f6cff1d2"));
...
unsigned char pchMessageStart[4] = { 0xb4, 0xd8, 0xe3, 0xe6 };
